...
import { Component, reactive, useRef, useState,onMounted } from "@odoo/owl";

const QUICK_CREATE_FIELD_TYPES = ["char", "boolean", "many2one", "selection", "many2many"];


export class FrappeGanttController extends Component {
    static template = `frappe_gantt_integration.FrappeGanttController`;
    static components = { Layout, MultiRecordViewButton, SearchBar, CogMenu };
    static props = {
        ...standardViewProps,
        defaultGroupBy: {
    ...
    setup() {
        ...
        this.rootRef = useRef("frappe_gantt");
    ...
    async initializeGantt() {
        ...
        // 2. 验证 Frappe Gantt 库已加载
        if (!window.Gantt) {
            console.error("Error: Frappe Gantt (window.Gantt) not loaded.");
            return;
        }
        ...
        // 4. 初始化甘特图
        this.gantt = new window.Gantt("#frappe_gantt", this.tasks, {
            view_mode: "Day",
            language : 'zh',// 默认视图
            container_height:'500',
            custom_popup_html: null  // 禁用默认弹窗（可选）
        });

        console.log("Gantt initialized:", this.gantt);
    }
